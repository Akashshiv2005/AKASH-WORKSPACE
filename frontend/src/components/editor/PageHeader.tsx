...
export const PageHeader: React.FC<PageHeaderProps> = ({ page }) => {
  ...
  return (
    <div className="relative group max-w-4xl mx-auto pt-6 px-4 sm:px-16">
      ...
      <div className="w-full overflow-hidden py-2 px-3.5 rounded-xl bg-white border border-amber-300 text-zinc-900 text-xs font-bold mb-4 shadow-xs flex items-center justify-between">
        <div className="flex items-center space-x-2.5 min-w-0 flex-1">
          <span className="px-2 py-0.5 rounded-md bg-gradient-to-r from-red-600 via-red-500 to-amber-500 text-white text-[10px] font-black uppercase tracking-wider shrink-0 shadow-xs">
            LIVE FOCUS
          </span>
          <TypewriterText
            phrases={[
            ...
            deletingSpeed={22}
            pauseDuration={2600}
            className="truncate font-bold text-zinc-900"
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex-1 relative flex items-start min-w-[240px]">
          <h1 className="w-full text-3xl sm:text-4xl lg:text-5xl font-black text-zinc-900 tracking-tight font-['Sora'] leading-tight py-1 select-text">
            {page.title || 'Untitled'}
          </h1>
        </div>
      </div>
